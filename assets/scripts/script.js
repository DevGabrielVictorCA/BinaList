...
const criarTarefaBtn = document.getElementById("criar-tarefa-btn");
...
const templateNenhumaTarefa = document.querySelector(".template-nenhuma-tarefa");
...
const reverterFiltrosBtn = document.getElementById("reverter-filtros-btn");
let textoDeErroContainer = document.querySelector(".texto-de-erro-container");
let textoDeErro = document.getElementById("texto-de-erro");
...
const scoreTarefa = { Alta: 100, Media: 50, Baixa: 10 };
function pegarIndexTarefa(id) {
    return arrayTarefas.findIndex((tarefa) => tarefa.id === id);
}
function pegarDatas() {
...
function pegarDataScore(dataTarefa) {
    ...
    if (dataTarefa < hoje) {
        const diasAtrasados = (new Date(hoje) - new Date(dataTarefa)) / (1000 * 60 * 60 * 24);
        return 30 + diasAtrasados * 5;
    }
    return 10;
}
function gerarTarefaScore(tarefa) {
    return (scoreTarefa[tarefa.prioridade] || 0) + pegarDataScore(tarefa.data);
}
function criarLiTarefa(tarefa, isDestaque = !1) {
    const clone = templateTarefa.content.cloneNode(!0);
    ...
    switch (tarefa.data) {
        case hoje:
            itemData.textContent = "Hoje";
            break;
        case amanha:
            itemData.textContent = "Amanhã";
            break;
        case ontem:
            itemData.textContent = "Ontem";
            break;
        default:
...
function estilizarTasks(isDestaque, li, tarefa) {
    if (isDestaque) {
        li.classList.add("tarefa-destaque", "ativo");
    }
...
function renderizarTarefaDestaque() {
    ...
    textoNenhumaTarefa(tarefasPendentes);
...
function erroFormulario(errorText, action) {
    ...
    textoDeErro.textContent = errorText;
    if (action === "add") {
        campoTarefa.classList.remove("erro-form");
        requestAnimationFrame(() => {
            campoTarefa.classList.add("erro-form");
        });
    } else if (action === "remove") {
        campoTarefa.classList.remove("erro-form");
        textoDeErroContainer.classList.remove("ativo");
        textoDeErro.textContent = "";
    }
}
function tarefaPassaNosFiltros(tarefa) {
    const { hoje, amanha } = pegarDatas();
    const categoriaSelecionada = filtros.categoria === "mostrar-todas" || tarefa.categoria === filtros.categoria;
    const prioridadeSelecionada = filtros.prioridade === "mostrar-todas" || tarefa.prioridade === filtros.prioridade;
    let dataSelecionada = !0;
    switch (filtros.data) {
        case "hoje":
            dataSelecionada = tarefa.data === hoje;
            break;
        case "amanha":
            dataSelecionada = tarefa.data === amanha;
            break;
        case "passadas":
            dataSelecionada = !!tarefa.data && tarefa.data < hoje;
            break;
    }
    let statusSelecionado = !0;
    switch (filtros.status) {
        case "concluidas":
            statusSelecionado = tarefa.completo;
            break;
        case "pendentes":
            statusSelecionado = !tarefa.completo;
            break;
...
function atualizarLabelFiltros() {
...
function enviarTarefa() {
    ...
    if (!campoTarefa.value.trim()) {
        erroFormulario("Digite um valor válido!", "add");
    ...
    if (tarefaEmEdicao !== null) {
        ...
        if (
            ...
            erroFormulario("Não houve edição", "add");
            clearTimeout(erroTimeout);
            erroTimeout = setTimeout(() => {
                erroFormulario("", "remove");
            }, 5000);
...
function barraDeProgresso() {
    const tarefasConcluidas = arrayTarefas.filter((t) => t.completo).length;
...
function exibirInformacoes(li) {
    ...
    tarefas.forEach((t) => t.classList.remove("ativo"));
...
function handleClickForaDoFormulario(e) {
    const addFiltros = document.getElementById("add-filtros-input");
    const clicouNoCheck = e.target.closest(".checkbox");
    const clicouDentroDoForm = createTaskForm.contains(e.target);
    const clicouEditar = e.target.closest('[data-function="editar"]');
    const clicouCriarTarefa = e.target.closest("#criar-tarefa-btn");
    if (clicouCriarTarefa) {
        campoTarefa.focus();
        createTaskForm.classList.add("ativo");
        return;
    }
    if (!clicouDentroDoForm && !clicouNoCheck && !clicouEditar && !criarTarefaBtn) {
        createTaskForm.classList.remove("ativo");
        erroFormulario("", "remove");
        limparCampos();
...
document.addEventListener("click", handleClickForaDoFormulario);
document.addEventListener("click", (e) => {
    ...
    if (!clicouNaArea) {
        filtrarTodasarefasBtn.checked = !1;
    }
});
campoTarefa.addEventListener("input", () => {
